...
| waveai:disabled                      | bool     | turns Wave AI off entirely (default false): the AI panel can't be opened (button, shortcut, `wsh`, "Send to Wave AI"), an open panel closes, and the backend refuses AI chat requests so nothing is sent to any AI provider. Can be toggled from the About dialog |
...
| conn:autoconnect                     | bool     | when true (default), at startup Wave connects every SSH host used by a terminal in an open window (not just the visible tab) and starts those shells in the background, retrying network failures with backoff for up to 30 minutes. Can be set per connection |
| conn:syncsshconfig                   | bool     | when true (default), every `Host` in `~/.ssh/config` / `/etc/ssh/ssh_config` (including `Include`d files) gets a stub entry in `connections.json` so per-host settings can be added there. Only adds entries; never edits or removes them                     |
| conn:showvitals                      | bool     | when true (default), each block's connection chip shows small live CPU and memory meters for connected SSH hosts (polled every few seconds over the existing connection, no agent) |
...
| term:tmux                                                           | bool     | for SSH connections without `wsh`, runs each terminal inside a per-block `tmux` session on the remote host so it survives Wave restarts and disconnects (defaults to false). See [Connections](./connections) |
| term:reconnectcmd                                                   | string   | for SSH connections without `wsh`, a command typed into each fresh session (with `term:tmux`, only when the tmux session is created). Usually set per connection |
...
| preview:dirsfirst                    | bool     | list folders before files in the file browser, whatever the sort column or direction (defaults to true) |
| preview:doubleclickopen              | string   | what double-clicking a file in the file browser does: `"external"` (default) opens it in the OS default application (Windows shows the "Open with" picker if nothing is associated), `"preview"` opens it inside Wave. Remote (SSH/WSL) files are downloaded to a temp folder, opened locally, and uploaded back to the host every time you save them; if the remote file changed since you opened it you are asked before it is overwritten |
| preview:externaleditor               | string   | path to the editor used by the "Open in …" file menu item (e.g. `C:\\Program Files\\Notepad++\\notepad++.exe`). When empty, Notepad++ is detected in its standard Windows install location |
...
| autoupdate:source                    | string   | where updates come from: "atreus" (this fork's builds, from https://www.atreusproject.com/updater/waveterm, the default) or "official" (upstream Wave Terminal builds — installing one replaces this fork's features). Can also be switched from the About dialog, which applies it immediately; edits to settings.json take effect on the next app start |
...
| telemetry:enabled                    | bool     | set to enable/disable telemetry (defaults to false in the Atreus fork; when off, the daily diagnostic ping and Wave's cloud AI modes are disabled too)                                                                                                                                                                                                                               |
