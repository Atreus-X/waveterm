...
- automatically: with `conn:syncsshconfig` (on by default), Wave adds an entry to `connections.json` for each ssh config `Host` (including hosts pulled in via `Include`). Entries are stubs, so `HostName`, `User`, keys etc. are still read from ssh config; existing entries are never modified
...
| conn:autoconnect | This boolean controls whether terminals using this connection are connected and started automatically when Wave launches (see the global `conn:autoconnect` setting). Unreachable hosts are retried with backoff; authentication or host-key failures are not retried. Defaults to true. |
...
| term:tmux | This boolean runs the remote shell inside a per-block `tmux` session when `wsh` is not in use (`conn:wshenabled` false). Restarting Wave or reconnecting re-attaches to the same session, so running programs survive. Closing the block kills the session. Falls back to a plain shell if `tmux` is not installed. Defaults to false. |
| term:reconnectcmd | A command typed into the terminal when a fresh session starts, e.g. `"cd ~/repos && clear && claude agents"`. With `term:tmux`, it runs only when the tmux session is created (not on re-attach); without `term:tmux`, it runs in every new shell. Only applies to connections not using `wsh`. |
...
To disable `wsh` for every connection, set `"conn:wshenabled": false` in `settings.json` instead (a per-connection value still overrides it).

#### Browsing files without wsh (SFTP)

On connections that run without `wsh`, the file browser (preview view) and file operations (open, save, rename, delete, new file/folder, copy between hosts) use SFTP over the same SSH connection instead, so nothing needs to be installed on the remote host. The remote `sshd` must have the `sftp` subsystem enabled (the OpenSSH default). As with `wsh`, copies are single-file only and limited to 32MB; copies between two different hosts are relayed through Wave.
