...
### v0.15.4 (Atreus fork) &mdash; Sep 25, 2026

One theme for the whole app, and a settings page you don't have to edit by hand.

- **App Theme** - New `app:theme` setting applies one theme to the whole app in every window: backgrounds, text, accents, borders and panels all take their colors from the chosen terminal theme. Terminals without their own `term:theme` follow it. Pick it from **App Theme (All Windows)** in the tab or tab bar right-click menu, or from a terminal's Themes menu
- **Visual Settings Editor** - Settings &gt; General now opens on a **Visual** tab with dropdowns, toggles and fields for the common settings (appearance, terminal, editor, Wave AI, behavior, connections, updates). Changes save immediately, and each setting has a tooltip explaining what it does, its default and its `settings.json` key
- The raw editor is now **Advanced Options (JSON)** and reloads the file when opened, so it shows changes made in the Visual tab
- The **Updates from** and **Wave AI** switches moved from the About dialog to the new settings page

### v0.15.3 (Atreus fork) &mdash; Sep 24, 2026

Drag-and-drop uploads, an About screen that shows the fork, and more privacy controls.

- **Drag & Drop Upload** - Drop files or folders from your desktop onto a file browser folder to copy them there (local, `wsh` and SFTP connections; SFTP hosts take files up to 32 MB). Existing names are collected into a single "Overwrite?" prompt
- **Folders First** - The file browser lists folders before files for any sort column or direction. Toggle it under Default Settings &gt; Folders First (`preview:dirsfirst`, on by default)
- **Fork & Update Source in About** - The About dialog shows which fork you're running and an "Updates from" toggle (Atreus fork / Official Wave) that writes `autoupdate:source` and switches the update feed immediately
- **Turn Off Wave AI** - New `waveai:disabled` setting (also an Enabled / Disabled toggle in About) hides the AI panel and its buttons and makes the backend refuse chat requests
- [privacy] Changing `telemetry:enabled` no longer reports the opt-in/opt-out to Wave's servers, links no longer carry `?ref=` tracking tags, and `web:defaulturl` points at the fork's repo
- [bugfix] In the file browser, Enter could open the wrong entry when the ".." row wasn't first in the sorted order

### v0.15.2 (Atreus fork) &mdash; Sep 24, 2026

Open files from the file browser in your own apps.

- **Double-click Opens in Default App** - Double-clicking a file in the file browser now opens it in the OS default application; on Windows, the "Open with" picker appears when no app is associated. Folders still open in the file browser. Set `preview:doubleclickopen` to `"preview"` to keep the old in-Wave preview
- **Edit Remote Files Locally** - Files on SSH/WSL connections are downloaded to a temp folder, opened locally, and uploaded back to the host every time you save. If the remote file changed since you opened it, you're asked before it's overwritten
- **Open in Notepad++** - New file menu items: "Open in Default Application", "Open in Notepad++" (auto-detected; set `preview:externaleditor` to use another editor), and "Open With…" on Windows

### v0.15.1 (Atreus fork) &mdash; Sep 23, 2026

Fixes for SSH connections that run without `wsh`.

- [bugfix] **Connection Degraded** - Connections without `wsh` no longer get stuck showing "Connection Degraded" after the first keystroke; connecting and terminal output now count as activity, so keepalives run and the status recovers
- [bugfix] **Durable Sessions** - Terminals on connections without `wsh` (disabled in config, or declined/failed at connect) now start a normal shell instead of a durable session that could never start (durable sessions need `wsh`'s job manager; use `term:tmux` for sessions that survive restarts)
- [bugfix] **File Browser** - SSH connections without `wsh` now appear in the file browser's connection picker (they are browsed over SFTP)

### v0.15.0 (Atreus fork) &mdash; Sep 23, 2026

First release of the [Atreus-X fork](https://github.com/Atreus-X/waveterm), based on upstream v0.14.5 plus later upstream fixes. Focused on using SSH hosts from `~/.ssh/config` **without installing `wsh`** on them.

- **tmux-backed Remote Sessions** - New `term:tmux` setting (global, per connection, or per block) runs each remote terminal inside its own tmux session on the host, so restarting Wave or losing the network re-attaches to the same session instead of starting over. Closing the block ends the session
- **Reconnect Command** - New `term:reconnectcmd` setting types a command into each fresh session (e.g. `cd ~/repos && clear && claude agents`); with tmux it runs only when the session is first created
- **Auto-connect on Launch** - New `conn:autoconnect` setting (default on) connects and starts remote terminals in _all_ tabs at startup, not just the visible one, and retries unreachable hosts with backoff
- **SFTP File Browser** - The file browser, previews, and file operations now work on connections without `wsh`, using SFTP over the existing SSH connection (nothing to install on the remote)
- **SSH Config Sync** - Hosts from `~/.ssh/config` (now including `Include`d files) are added to `connections.json` automatically (`conn:syncsshconfig`, default on) so per-host settings have a place to live; existing entries are never modified
- **Update Source** - New `autoupdate:source` setting chooses between this fork's builds (`atreus`, default) and upstream Wave (`official`)
- **Unofficial Build Identity** - This build uses its own app ID (`com.atreusproject.waveterm`) and author, ships Wave's `LICENSE` and `NOTICE`, and says in About that it is an unofficial build not endorsed by Command Line Inc.
- **Telemetry Off by Default** - `telemetry:enabled` now defaults to false; with it off, the daily diagnostic ping to Wave's servers is skipped and Wave's hosted cloud AI modes are hidden (`waveai:showcloudmodes: false`) — use your own AI provider keys instead
- [bugfix] System SSH config is read from `/etc/ssh/ssh_config` (was `/etc/ssh/config`)
- [bugfix] Connections from `connections.json` are listed in a stable order in the connection picker
- [docs] Fixed the documented key for disabling `wsh` (`conn:wshenabled`)
- Package updates (`golang.org/x/crypto`, `github.com/pkg/sftp`)
